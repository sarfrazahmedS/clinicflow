import { AppointmentStatus, Gender, PrescriptionStatus, PrismaClient, Role } from "@prisma/client";
...
async function resetDatabase() {
  // Demo database only — wipe in FK-safe order for a deterministic seed.
...
async function buildClinic(opts: {
  ...
  const clinic = await prisma.clinic.create({
    data: {
      ...
      phone: "+1 555 0100",
      address: "123 Health Street, Suite 200",
    },
  });

  await prisma.user.create({
    data: { clinicId: clinic.id, email: `admin@${slug}.dev`, name: `${name} Admin`, role: Role.CLINIC_ADMIN, passwordHash },
  });
  const drSmith = await prisma.user.create({
    data: { clinicId: clinic.id, email: `dr.smith@${slug}.dev`, name: "Dr. Sarah Smith", role: Role.DOCTOR, specialty: "General Medicine", licenseNo: "LIC-1001", passwordHash },
  });
  const drKhan = await prisma.user.create({
    data: { clinicId: clinic.id, email: `dr.khan@${slug}.dev`, name: "Dr. Imran Khan", role: Role.DOCTOR, specialty: "Cardiology", licenseNo: "LIC-1002", passwordHash },
  });
  await prisma.user.create({
    data: { clinicId: clinic.id, email: `reception@${slug}.dev`, name: `${name} Reception`, role: Role.STAFF, passwordHash },
  });
  ...
  // Give the first patient a portal login (role PATIENT).
  ...
  // A completed visit (past) with a record + prescription, and upcoming bookings.
...
async function main() {
  ...
  await buildClinic({ name: "Sunrise Medical Center", slug: "sunrise", brandColor: "#2563eb", passwordHash });
  await buildClinic({ name: "Downtown Family Clinic", slug: "downtown", brandColor: "#059669", passwordHash });
